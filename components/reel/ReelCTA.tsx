  ...
  return (
    <div className="pointer-events-auto flex items-center justify-between gap-3">
      <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-2 gap-y-1">
        <ProductPrice
          amount={amount}
          className="min-w-0 max-w-full truncate text-base font-semibold text-white"
          ctaType={ctaType}
          currency={currencyCode || "INR"}
          displayCurrency={displayCurrency}
        />
        {isPaid ? (
          <div className="shrink-0 [&_label]:gap-1 [&_select]:h-7 [&_select]:rounded-full [&_select]:border-white/15 [&_select]:bg-black/45 [&_select]:px-2 [&_select]:text-[11px] [&_span]:sr-only">
            <CurrencySelector
