...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-[max(0.7rem,env(safe-area-inset-bottom))] left-1/2 z-50 flex w-[calc(100%-1rem)] max-w-[26rem] -translate-x-1/2 items-center justify-around rounded-[1.75rem] border border-white/10 bg-slate-950/68 px-1.5 py-1.5 text-white shadow-[0_16px_42px_rgba(0,0,0,0.42)] backdrop-blur-2xl">
      {tabs.map((tab) => {
        ...
        return (
          <Link
            key={tab.href}
            href={tab.href}
            className={`relative flex min-w-0 flex-1 flex-col items-center gap-0.5 rounded-[1.35rem] px-1.5 py-1.5 text-[10.5px] font-semibold transition active:scale-95 ${
              isActive ? "text-cyan-300" : "text-white/55 hover:bg-white/[0.035] hover:text-white"
            }`}
          >
            <NavIcon icon={tab.icon} />
            <span>{tab.label}</span>
            {isActive ? <span className="absolute bottom-0.5 h-0.5 w-4 rounded-full bg-cyan-300/80" /> : null}
          </Link>
        );
      })}
    </nav>
